...
class DatasetFPHA(Dataset):
    ...
    def prepare_data(self):
        ...
        self.history_caption = torch.load("models/text_model/FPHA_caption_simple_en_tensor.p", weights_only=True)
    ...
    def process_data(self):
        ...
        if self.multimodal_path is None:
            self.data_multimodal = np.load(
                "dataset/FPHA_multi_modal/t_his15_1_thre0.050_t_pred60_thre0.100_index_subAll_random.npz",
                allow_pickle=True,
            )["data_multimodal"].item()
            data_candi = np.load(
                "dataset/FPHA_multi_modal/data_multimodal_t_his15_t_pred60_skiprate10_random.npz",
                allow_pickle=True,
            )["data_candidate.npy"]
        else:
    ...
    def sampling_generator(self, num_samples=5000, batch_size=16, n_modality=10):
        for i in range(num_samples // batch_size):
            ...
            for i in range(batch_size):
                sample_i, sample_multi_i, action, caption, fr_start = self.sample(n_modality=n_modality)
                sample.append(sample_i)
                sample_multi.append(sample_multi_i[None, ...])
                actions.append(action.split("_")[2][:-2])
