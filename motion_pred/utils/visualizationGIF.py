...
def render_animationGIF(skeleton, poses_generator, t_hist, fix_0=True, azim=0.0, output=None, size=6, ncol=12, scale=1):
    """
    ...
    trajectories = []
    radius = 2
    for index, (_, data) in enumerate(poses.items()):
    ...
    fig.tight_layout()
    fig.subplots_adjust(wspace=-0.2, hspace=0.05, left=0.01, right=0.99, bottom=0.01, top=0.99)
    ...
    def save():
        ...
        fps = 24
        anim = animation.FuncAnimation(fig, update_video, frames=np.arange(0, poses[0].shape[0]), interval=1000 / fps, repeat=False)
        os.makedirs(output + "video", exist_ok=True)
        writer_gif = animation.PillowWriter(fps=fps)
        anim.save(f"{output}video/{action}_{fr_start}s_PromptFDDMv2.gif", writer=writer_gif)

        # writervideo = animation.FFMpegWriter(fps=fps)
        # anim.save(f"{output}video/{action}_{fr_start}s_PromptFDDMv2.mp4", writer=writervideo)
        tqdm.write(f"video saved to {output}video/{action}_{fr_start}s_PromptFDDMv2.mp4!")
