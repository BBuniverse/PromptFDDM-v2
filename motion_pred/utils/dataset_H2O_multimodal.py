...
class DatasetH2O(Dataset):
    ...
    def prepare_data(self):
        self.data_file = "dataset/H2O_all_split_full.p"
        self.history_caption_all = torch.load("models/text_model/H2O_caption_full_simple_en_tensor.p", weights_only=True)
    ...
    def process_data(self):
        ...
        if self.multimodal_path is None:
            self.data_multimodal = np.load(
                "dataset/H2O_multi_modal/t_his20_1_thre0.050_t_pred80_thre0.100_index_subAll_random.npz",
                allow_pickle=True,
            )["data_multimodal"].item()
            data_candi = np.load(
                "dataset/H2O_multi_modal/data_multimodal_t_his20_t_pred80_skiprate15_random.npz",
                allow_pickle=True,
            )["data_candidate.npy"]
        else:
    ...
    def sample(self, n_modality=10):
        ...
        # subject = "subject4"
        # sampleGIF = {
        #     "subject4/k2/7/cam4/sub_1": 359,
        # }
        # dict_s = self.data[subject]

        # action = np.random.choice(list(sampleGIF.keys()))
        # seq = dict_s[action]
        # fr_start = sampleGIF[action]
        # fr_end = fr_start + self.t_total
        # traj = seq[fr_start:fr_end]
        ...
        if n_modality > 0 and subject in self.data_multimodal.keys():
            ...
            return traj[None, ...], traj_multi, action.replace("/", "-"), caption, fr_start
        else:
            return traj[None, ...], None, action.replace("/", "-"), caption, fr_start
        ...
        for sub in self.data.keys():
            ...
            for act in data_s.keys():
                ...
                for i in range(0, seq_len - self.t_total, step):
                    ...
                    yield traj, traj_multi, act.replace("/", "-"), caption
