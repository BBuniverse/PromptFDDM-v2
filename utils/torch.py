...
def get_scheduler(optimizer, policy, nepoch_fix=None, nepoch=None, decay_step=None):
    if policy == "lambda":
        ...
        def warmup_and_decay(current_step):
            if current_step < nepoch_fix:
                return float(current_step) / float(max(1, nepoch_fix))
            else:
                progress = float(current_step - nepoch_fix) / float(max(1, nepoch - nepoch_fix))
